...
namespace TestGenerateAdminCode
{
    ...
    public class SourceFilesGenerator
    {
        ...
        public void GenerateFiles()
        {
            ...
            foreach (var singleDbSet in dbSets)
            {
                ...
                string blazorEditPageCode = GenerateBlazorEditPageCode(dataAccessAssembly, entityType, entityProperties, entitTypes, this._sourceFilesGeneratorConfiguration.BasePagesRoute);
                ...
                TypeMapping typeMapping = new TypeMapping()
                {
                    ...
                    BlazorCrud = new BlazorCrud()
                    {
                        ...
                        EditPageCodeString = blazorEditPageCode
                    },
            ...
            foreach (var singleMappedType in mappedTypes)
            {
                ...
                string blazorEditPageFileName = Path.Combine(this._sourceFilesGeneratorConfiguration.BlazorFilesDestinationFolder, @$"{singleMappedType.SourceTypeName}", $"Edit.razor");
                File.WriteAllText(blazorEditPageFileName, singleMappedType.BlazorCrud.EditPageCodeString);
        ...
        private static string GenerateBlazorEditPageCode(Assembly dataAccessAssembly, Type entityType, PropertyInfo[] entityProperties,List<Type> entitTypes, string basePageRoute = "/AutogeneratedPages")
        {
            PropertyInfo keyProperty = FindKeyProperty(entityProperties);
            CodeWriter codeWriter = new CodeWriter();
            codeWriter.WriteLine($"@page \"{basePageRoute}/{entityType.Name}/Edit/{{{keyProperty.Name}}}\"");
            codeWriter.WriteLine();
            codeWriter.WriteLine("<EditForm Model=\"@_model\" OnValidSubmit=\"OnValidSubmit\">");
            codeWriter.WriteLine("<DataAnnotationsValidator></DataAnnotationsValidator>");
            codeWriter.WriteLine("<ValidationSummary></ValidationSummary>");
            ...
            codeWriter.WriteLine($"public String {keyProperty.Name} {{get;set;}}");
            codeWriter.WriteLine($"public {entityType.Name}Model _model {{get;set;}}=new {entityType.Name}Model();");
            codeWriter.WriteLine("[Inject]");
            codeWriter.WriteLine("private HttpClient _httpClient {get;set;}");
            codeWriter.WriteLine("[Inject]");
            ...
            codeWriter.WriteLine("{");
            codeWriter.WriteLine($"string requestUrl = $\"api/{entityType.Name}/Get{entityType.Name}ById?{keyProperty.Name.ToLower()}={{{keyProperty.Name}}}\";");
            codeWriter.WriteLine($"this._model = await this._httpClient.GetFromJsonAsync<{entityType.Name}Model>(requestUrl);");
            codeWriter.WriteLine("}");
            codeWriter.WriteLine("private async Task OnValidSubmit()");
            codeWriter.WriteLine("{");
            codeWriter.WriteLine($"string requesturl = \"api/{entityType.Name}/Edit{entityType.Name}\";");
            codeWriter.WriteLine($"var response = await this._httpClient.PutAsJsonAsync<{entityType.Name}Model>(requesturl, _model);");
            codeWriter.WriteLine("if (response.IsSuccessStatusCode)");
            codeWriter.WriteLine("{");
            codeWriter.WriteLine($"_navigationManager.NavigateTo(_navigationManager.Uri.Replace($\"{entityType.Name}/Edit/{{{keyProperty.Name}}}\", \"{entityType.Name}/List\"));");
            codeWriter.WriteLine("}");
            codeWriter.WriteLine("}");
            codeWriter.WriteLine("}");
            var blazorPageCode = codeWriter.GenerateCode();
            ...
            string dbContextName, string dbContextNamespace, string entitiesNamespace)
        {
            ...
            GenerateGetByIdEndpoint(entityType, dbContextName, strBuilder);
            GenerateEditEndpoint(entityType, dbContextName, strBuilder);
            strBuilder.AppendLine("}");
            var csCode = CSharpSyntaxTree.ParseText(strBuilder.ToString()).GetRoot().NormalizeWhitespace().ToFullString();
            return csCode;
        }
        ...
        private static void GenerateGetByIdEndpoint(Type entityType, string dbContextName, StringBuilder strBuilder)
        {
            var entityProperties = entityType.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
            PropertyInfo keyProperty = FindKeyProperty(entityProperties);

            strBuilder.AppendLine("/// <summary>");
            strBuilder.AppendLine($"/// Retrieves the {entityType.Name.Humanize()} by its id");
            strBuilder.AppendLine("/// </summary>");
            strBuilder.AppendLine($"/// <returns>An object of <see cref=\"{entityType.Name}\"/> </returns>");
            strBuilder.AppendLine("[HttpGet(\"[action]\")]");
            strBuilder.AppendLine($"public async Task<{entityType.Name}Model> Get{entityType.Name}ById({keyProperty.PropertyType.Name} {keyProperty.Name.ToLower()})");
            strBuilder.AppendLine("{");
            strBuilder.AppendLine($"var entity = await this._{dbContextName.ToLower()}.{entityType.Name}.SingleAsync(p=>p.{keyProperty.Name} == {keyProperty.Name.ToLower()});");
            strBuilder.AppendLine($"return this._mapper.Map<{entityType.Name}, {entityType.Name}Model>(entity);");
            strBuilder.AppendLine("}");
        }

        private static void GenerateEditEndpoint(Type entityType, string dbContextName, StringBuilder strBuilder)
        {
            var entityProperties = entityType.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly);
            PropertyInfo keyProperty = FindKeyProperty(entityProperties);

            strBuilder.AppendLine("/// <summary>");
            strBuilder.AppendLine($"/// Edit the {entityType.Name.Humanize()} with the given id");
            strBuilder.AppendLine("/// </summary>");
            strBuilder.AppendLine($"/// <returns>An object of <see cref=\"{entityType.Name}Model\"/> </returns>");
            strBuilder.AppendLine("[HttpPut(\"[action]\")]");
            strBuilder.AppendLine($"public async Task<{entityType.Name}Model> Edit{entityType.Name}({entityType.Name}Model {entityType.Name.ToLower()}Model)");
            strBuilder.AppendLine("{");
            strBuilder.AppendLine($"var entity = this._mapper.Map<{entityType.Name}Model,{entityType.Name}>({entityType.Name.ToLower()}Model);");
            strBuilder.AppendLine($"this._{dbContextName.ToLower()}.{entityType.Name}.Update(entity);");
            strBuilder.AppendLine($"await this._{dbContextName.ToLower()}.SaveChangesAsync();");
            strBuilder.AppendLine($"return this._mapper.Map<{entityType.Name}, {entityType.Name}Model>(entity);");
            strBuilder.AppendLine("}");
        }

        private static PropertyInfo FindKeyProperty(PropertyInfo[] entityProperties)
        {
            return entityProperties.Where(p => p.GetCustomAttribute<KeyAttribute>() != null).Single();
        }
    ...
    public class BlazorCrud
    {
        ...
        public string EditPageCodeString { get; internal set; }
    }
